...
export default function ARAgingPage() {
  ...
  const [data, setData] = useState<ARReportData | null>(null);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await apiFetch('/api/reports?type=accounts-receivable');
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {loading ? (
        ...
        <>
          ...
          {grandTotal === 0 && (
            <div className="card text-center py-12 text-corporate-gray">No outstanding receivables.</div>
          )}
        </>
      ) : (
        <div className="card text-center py-12 text-corporate-gray">Unable to load A/R aging.</div>
      )}
    </div>
  );
}